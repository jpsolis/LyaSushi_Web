...
function ejecutarAccion(accion, idApanado, descripcionApanado, precioUnitario, precio, cantidad)
{
    ...
    if(accion == "Editar")
    {
        ...
        let url = "http://localhost/LyaSushi_Api_v2/Apanado/api/Apanado/Edit";
        let registro = {
            "idApanado": idApanado,
        ...
        let url = "http://localhost/LyaSushi_Api_v2/Apanado/api/Apanado/Delete/" +idApanado;
        console.log("URL: " +url);
       ...
       location.reload();
        ...
        let url = "http://localhost/LyaSushi_Api_v2/Apanado/api/Apanado/Create";
        console.log("URL: " +url);
...
function cargarGrilla()
{
    ...
    const tabla = document.querySelector('#tablaApanados tbody')

    fetch('http://localhost/LyaSushi_Api_v2/Apanado/api/Apanado/Get')
    ...
    .then((data) => {
     ...
     data.forEach(element => {
        ...
        fila.innerHTML += `
        <td>${element.idApanado}</td>
        <td>${element.descripcionApanado}</td>        
        <td>${element.precioUnitario}</td>        
        <td>${element.precio}</td>        
        <td>${element.cantidad}</td>        
        <td><a href="#" id="btnEditar" onclick="modalEditar('${element.idApanado}', '${element.descripcionApanado}', '${element.precioUnitario}', '${element.precio}', '${element.cantidad}', 'Editar')">  <i class="fa fa-edit"></i></a></td>
        <td><a href="#" id="btnEliminar" onclick="modalEditar('${element.idApanado}', '${element.descripcionApanado}', '${element.precioUnitario}', '${element.precio}', '${element.cantidad}','Eliminar')"><i class="fa fa-trash"></i></a></td>
        `;
    ...
    .then((data) => {
     console.log("lalala " +data);
      for(let i = 0; i < data.length; i++){
        ...
        console.log("Descripcion: " +data[i].descripcionApanado)
